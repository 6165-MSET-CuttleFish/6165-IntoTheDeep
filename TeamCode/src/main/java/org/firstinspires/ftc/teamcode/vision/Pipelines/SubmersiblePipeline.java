...
public class SubmersiblePipeline extends OpenCvPipeline {
    ...
    public Point biggestArea(SampleColor a) {
        DetectedSample biggestContour = new DetectedSample(SampleColor.NEUTRAL, new RotatedRect(), 0);
        for (DetectedSample sample : detectedSamples) {
            biggestContour = sample.getArea() > biggestContour.getArea() && sample.color == a ? sample : biggestContour;
        }

        return biggestContour.orientedRect.center;
    }
}
